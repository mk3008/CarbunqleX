...
public class SelectQueryParserTests
{
    ...
    [InlineData("array cast", "select array[1, 2, 3]::integer[]")]
    [InlineData("array cast as", "select cast(array[1, 2, 3] as integer[])")]
    ...
    [InlineData("Postgres named parameters", "select id, name from users where id = :id and age > :age")]
    [InlineData("Postgres positional parameters", "select id, name from users where id = $1 and age > $2")]
