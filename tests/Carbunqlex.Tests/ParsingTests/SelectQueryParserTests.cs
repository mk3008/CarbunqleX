...
public class SelectQueryParserTests
{
    ...
    //[InlineData("Trim with BOTH (default) and characters", "select trim('xyz' from 'yxTomxx')")]
    //[InlineData("Trim with LEADING and characters", "select trim(leading 'xyz' from 'yxTomxx')")]
    //[InlineData("Trim with TRAILING and characters", "select trim(trailing 'xyz' from 'yxTomxx')")]
    //[InlineData("Trim with BOTH (default) and default characters", "select trim(from '  yxTomxx  ')")]
    //[InlineData("Postgres Trim with BOTH (default) from and characters", "select trim(both from 'yxTomxx', 'xyz')")]
    //[InlineData("Postgres Trim with LEADING from and characters", "select trim(leading from 'yxTomxx', 'xyz')")]
    //[InlineData("Postgres Trim with TRAILING from and characters", "select trim(trailing from 'yxTomxx', 'xyz')")]
    //[InlineData("Postgres Trim with BOTH (default) from and default characters", "select trim(from '  yxTomxx  ')")]
    //[InlineData("Postgres Trim with default BOTH and characters", "select trim('yxTomxx', 'xyz')")]
    //[InlineData("Postgres Trim with default BOTH and default characters", "select trim('  yxTomxx  ')")]
    [InlineData("Concatenate strings", "select concat('Hello', ' ', 'World', '!')")]
    [InlineData("Substring", "select substring('Thomas', 2, 3)")]
    [InlineData("Substring with FROM and FOR", "select substring('Thomas' from 2 for 3)")]
    [InlineData("Substring with only FROM", "select substring('Thomas' from 3)")]
    [InlineData("Substring with only FOR", "select substring('Thomas' for 2)")]
    [InlineData("Substring with FROM using regex pattern", "select substring('Thomas' from '...$')")]
    [InlineData("Substring with SIMILAR pattern", "select substring('Thomas' similar '%#\"o_a#\"_' escape '#')")]
    [InlineData("String replace", "select replace('Hello World', 'World', 'PostgreSQL')")]
    [InlineData("Regex replace", "select regexp_replace('Hello 123 World', '\\d+', '456')")]
    [InlineData("Position in string", "select position('World' in 'Hello World')")]
    [InlineData("Overlay with FROM and FOR", "select overlay('Txxxxas' placing 'hom' from 2 for 4)")]
    [InlineData("Overlay with FROM and without FOR", "select overlay('Txxxxas' placing 'hom' from 2)")]
    [InlineData("is distinct from", "select NULL is distinct from NULL")]
    [InlineData("is not distinct from", "select NULL is not distinct from NULL")]
    //[InlineData("IS NORMALIZED (default NFC)", "select U&'\\0061\\0308bc' IS NORMALIZED")]
    //[InlineData("IS NOT NORMALIZED (default NFC)", "select U&'\\00E4bc' IS NOT NORMALIZED")]
    //[InlineData("IS NFC NORMALIZED", "select U&'\\00E4bc' IS NFC NORMALIZED")]
    //[InlineData("IS NOT NFC NORMALIZED", "select U&'\\0061\\0308bc' IS NOT NFC NORMALIZED")]
    //[InlineData("IS NFD NORMALIZED", "select U&'\\0061\\0308bc' IS NFD NORMALIZED")]
    //[InlineData("IS NOT NFD NORMALIZED", "select U&'\\00E4bc' IS NOT NFD NORMALIZED")]
    //[InlineData("IS NFKC NORMALIZED", "select U&'\\FB01' IS NFKC NORMALIZED")]
